...
class ChangeFontUseCaseTest {
    ...
    @InjectMocks
    private ChangeFontUseCase useCase;

    @Test
    void ChangeFontUseCaseTest() {

        ChangeFontCommand command = new ChangeFontCommand(
                "77777",
                "33333",
                "ComicSans"
        );

        FontChanged event = new FontChanged(
                "33333",
                "ComicSans"
        );
        ...
        StepVerifier.create(triggeredevents)
                ...
                .expectNextMatches(domainEvents ->
                        domainEvents.size() == 1 &&
                                domainEvents.get(0) instanceof FontChanged &&
                        ((FontChanged) domainEvents.get(0)).getCommentId().equals("33333")&&
                                ((FontChanged) domainEvents.get(0)).getFont().equals("ComicSans")
                )
                .verifyComplete();
