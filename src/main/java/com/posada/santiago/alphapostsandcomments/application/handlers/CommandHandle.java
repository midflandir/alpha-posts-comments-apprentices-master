...
import com.posada.santiago.alphapostsandcomments.business.usecases.ChangeFontUseCase;
...
public class CommandHandle {
    ...
    @Bean
    public RouterFunction<ServerResponse> changeFont(ChangeFontUseCase useCase){

        return route(
                PUT("/Comment/ChangeFont").and(accept(MediaType.APPLICATION_JSON)),
                request -> ServerResponse.ok().contentType(MediaType.APPLICATION_JSON)
                        .body(BodyInserters.fromPublisher(useCase.apply(request.bodyToMono(ChangeFontCommand.class)), DomainEvent.class))
        );
    }
}
